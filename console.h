...
struct Console
{
    ...
    void    UpdateDigestBuffer(std::string input)
    {
        digestBuffer = input;
    }
    ...
    {
        ImGui::SetNextWindowSize(ImVec2(650,255), ImGuiSetCond_FirstUseEver);
        ImGui::SetNextWindowPos(ImVec2(280,430), ImGuiSetCond_FirstUseEver);
        if (!ImGui::Begin(title, opened))
        {
            ImGui::End();
            return;
        }
        ImGui::Text("Digest buffer: ");
        ImGui::TextWrapped(digestBuffer.c_str());
